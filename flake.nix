{
  ...
  let
    ...
    image_dir = "image";
    /*
    template_main = "./template/main.typ";
    */
  in {
    devShells.${system} = {
      default = pkgs.mkShell {
        packages = [ pkgs.typst pkgs.unzip pkgs.jaq ];
        shellHook = ''
        '';
      };
    };
    ...
    packages.${system} = {
      ...
      compile = pkgs.writeShellApplication {
        ...
        runtimeInputs = [pkgs.typst];
        text = ''
          #tmpfile="result123459876.typ"
          #cat text/*typ > $tmpfile && typst compile --font-path . $tmpfile result.pdf
          #rm $tmpfile
